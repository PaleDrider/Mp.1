...
const Game = {
  ...
  // ─── Ekranų keitimas ───
  // Naudoja tik inline stilius — ignoruoja CSS klases kad išvengti !important konfliktų
  show(name) {
    ...
    if (!el) {
      console.error('Ekranas nerastas: scr-' + name);
      return;
    }
  ...
  _nextBit() {
    ...
    if (!available.length) {
      // Parodyti klaidą ekrane — ne tyliai
      const el = document.getElementById('scr-code');
      if (el) {
        el.style.display = 'flex'; el.style.opacity = '1'; el.style.pointerEvents = 'all';
        const box = el.querySelector('.parchment-box');
        if (box) box.innerHTML = '<div class="deco-div"></div><h2 style="color:var(--red-dot)">Klaida</h2><p style="color:var(--ink-2);margin:1rem 0">Istorijų failai neįkelti.<br><br>Patikrinkite:<br>• story_1.js įkeltas į GitHub<br>• SW cache atnaujintas (sw.js versija pakeista)</p><button onclick="location.reload(true)" class="rpg-btn full">↻ Perkrauti</button>';
      }
      return;
    }
  ...
  _showOv(id) {
    ...
    if (e) { e.style.display = 'flex'; e.classList.remove('hidden'); }
  },
  _hideOv(id) {
    const e = document.getElementById(id);
...
document.addEventListener('DOMContentLoaded', () => {
  document.addEventListener('click', () => { try { Sound.init(); } catch(e){} }, {once:true});

  const storyCount = Object.keys(window.STORIES || {}).length;
  console.log('Istorijų įkelta:', storyCount);
  if (!storyCount) {
    console.warn('Nėra istorijų — patikrinkite story_1.js failą');
  }

  try {
    Game.init();
    console.log('Game.init() sėkmingai');
  } catch(e) {
    console.error('Game.init() klaida:', e);
  }
});
